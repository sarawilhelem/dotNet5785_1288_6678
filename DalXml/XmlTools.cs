...
static class XMLTools
{
    ...
    {
        //function to get elment of TimeSpan type from xml file
        XElement root = XMLTools.LoadListFromXMLElement(xmlFileName);
    ...
    {
        //function to set element of TimeSpan type in xml file
        XElement root = XMLTools.LoadListFromXMLElement(xmlFileName);
        root.Element(elemName)?.SetValue((elemVal).ToString());
        XMLTools.SaveListToXMLElement(root, xmlFileName);
    }
    ...
    public static TimeSpan? ToTimeSpanNullable(this XElement element, string name) =>
        TimeSpan.TryParse((string?)element.Element(name), out var result) ? (TimeSpan?)result : null;   //function to try convert the element to TimeSpan type
